...
# This file builds the CF-AR models and computes the R squared values for the test set
...
mixed_mod_1 <- lmer(y_1 ~ 1 + `Level 4 Prop Change` + inflation + avg_unemployment + `No Qualifications Prop`
                       ...
                       + `Private Prop` + `X65. Change` + `X18.29 Change` + X50.64 + `Median Income`
                       + `No Qualifications Prop`:`No Qualifications Prop Change` + X18.29:`X18.29 Change`
                       + `Private Prop Change`:`Private Prop`
                       + (1 | Year), 
...
mixed_mod_2 <- lmer(y_2 ~ 1 + `No Qualifications Prop Change` + inflation + `Owner Prop Change` + `No RFM Before`
...
mixed_mod_3 <- lmer(y_3 ~ 1 + `Median Income Change` + `Level 4 Prop Change` + `No Qualifications Prop Change`
                       + `Median Income` + `Owner Prop Change` + inflation + X50.64
                       + child_poverty + `X65. Change` + Proportion + `X18.29 Change` + White + X65.
                       + `Social Prop` + home_prices + `No Qualifications Prop` + `Level 4 Prop` + `House Prices Change`
                       + `No RFM Before` + `Private Prop Change` + `No Qualifications Prop Change`:`No Qualifications Prop` 
...
mixed_mod_4 <- lmer(y_4 ~ 1 + inflation + `No RFM Before` + `No Qualifications Prop Change` + `No Qualifications Prop`
                       + `Level 4 Prop` + White + `Level 4 Prop Change` + `House Prices Change` + `White Change`
                       + `X65. Change` + X18.29 + Proportion + X50.64 + avg_unemployment + `Owner Prop` + `Median Income`
                       + `Owner Prop Change` + `X50.64 Change`
...
mixed_mod_5 <- lmer(y_5 ~ 1 + Proportion + X50.64 + `No Qualifications Prop` + `House Prices Change` + `Median Income`
                       + avg_unemployment + `Level 4 Prop` + `No RFM Before` + `X50.64 Change` + inflation
                       + `Median Income Change` + `Social Prop Change` + X18.29 + `X18.29 Change`
                       + (1 | Year),
...
grn_change <- predict(mixed_mod_1, newdata = election_data_testing)
grn <- election_data_testing$`GRN Share` + grn_change

rfm_change <- predict(mixed_mod_2, newdata = election_data_testing)
rfm <- election_data_testing$`RFM Share` + rfm_change

ld_change <- predict(mixed_mod_3, newdata = election_data_testing)
ld <- election_data_testing$`LD Share` + ld_change

con_change <- predict(mixed_mod_4, newdata = election_data_testing)
con <- election_data_testing$`Con Share` + con_change

lab_change <- predict(mixed_mod_5, newdata = election_data_testing)
lab <- election_data_testing$`Lab Share` + lab_change

# Set vote shares in seats where a party did not run to zero
for (i in 1:440) {
...
# Adjust vote shares to ensure sum to 100 constraint is met
...
cf_ar_model_plot <- grid.arrange(con_plot, lab_plot, ld_plot, rfm_plot, grn_plot, legend, ncol = 2, nrow = 3,
                                  layout_matrix = rbind(c(1,2), c(3,4), c(5,6)),
                                  widths = c(2, 2), heights = c(4, 4, 4))

ggsave('CF-AR Model Vote Share.png', cf_ar_model_plot, width = 15.66, height = 12, units = 'cm')

# Compute R squared values for each party
...
cf_ar_rsq <- c(con_rsq, lab_rsq, ld_rsq, rfm_rsq, grn_rsq)
cf_ar_rsq


# Create data frame for change in vote share of test set
...
# Combine plots into a single figure
cf_ar_model_change_plot <- grid.arrange(con_plot, lab_plot, ld_plot, rfm_plot, grn_plot, legend, ncol = 2, nrow = 3,
                                         layout_matrix = rbind(c(1,2), c(3,4), c(5,6)),
                                         widths = c(2, 2), heights = c(4, 4, 4))

ggsave('CF-AR Model Change.png', cf_ar_model_change_plot, width = 15.66, height = 12, units = 'cm')

# R squared for change in vote share
...
cf_ar_rsq_change <- c(con_rsq_change, lab_rsq_change, ld_rsq_change, rfm_rsq_change, grn_rsq_change)
cf_ar_rsq_change
...
covariate.labels <- c('Intercept', '18 to 29', '30 to 49', '50 to 64', '65+', 'Change in 18 to 29',
                          ...
                          'No Qual. Int.', 'No Qual. Int. 2', 'Median Income Int.', 'Median House Price Int.', 
                          'Ethnicity Int.', 'Home Owners Int.', 'Private Housing Int.')
...
covariate.stargazer <- c('Intercept', 'Change in Degree', 'Inflation', 'Unemployment', 'Home Owners',
                         ...
                         'Fuel Poverty', 'No RFM Before', 'Social Housing', 'Home Owners Change', 'Median House Price Change',
                         'Private Housing Change', 'Change in Ethnicity', 'No Qual. Int.', 'Degree Int.',
                         'Median Income Int.', 'Median House Price Int.', 'Degree Int. 2', 'Ethnicity Int.',
                         'Home Owners Int.', '50 to 64 Int.', 'Change in No Qual.', 'Ethnicity', '65+',
                         'Median House Price', '18 to 29', 'Child Poverty', 'Median Age', 'Private Housing',
                         'Change in 65+', 'Change in 50 to 64', 'Change in 18 to 29', '50 to 64', 'Median Income',
                         'No Qual. Int. 2', '18 to 29 Int.', 'Private Housing Int.')

covariate.order <- match(covariate.labels, covariate.stargazer)
covariate.order

stargazer(mixed_mod_1, mixed_mod_2, mixed_mod_3, mixed_mod_4, mixed_mod_5, align = TRUE, omit.stat=c("LL","ser","f"), no.space = TRUE,
