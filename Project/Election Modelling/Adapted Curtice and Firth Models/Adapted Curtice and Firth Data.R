...
# This file produces a data frame for the adapted CF models
...
# Load general election results on new boundaries and Blue Wall seats
...
# Compute vote shares for each party with votes for Other removed for each election
...
# Compute national vote shares for England for each major party for each election
...
response_2015 <- data.frame(matrix(nrow = 543, ncol = 7))
names(response_2015) <- c('Constituency', 'Region', 'y_1', 'y_2', 'y_3', 'y_4', 'y_5')
...
response_2015$y_2 <- results_2015$`RFM Share` - results_2010$`RFM Share`
response_2015$y_3 <- results_2015$`LD Share` - results_2010$`LD Share`
response_2015$y_4 <- results_2015$`Con Share` - results_2010$`Con Share`
response_2015$y_5 <- results_2015$`Lab Share` - results_2010$`Lab Share`

# Compute responses for 2017
response_2017 <- data.frame(matrix(nrow = 543, ncol = 7))
names(response_2017) <- c('Constituency', 'Region', 'y_1', 'y_2', 'y_3', 'y_4', 'y_5')
...
response_2017$y_2 <- results_2017$`RFM Share` - results_2015$`RFM Share`
response_2017$y_3 <- results_2017$`LD Share` - results_2015$`LD Share`
response_2017$y_4 <- results_2017$`Con Share` - results_2015$`Con Share`
response_2017$y_5 <- results_2017$`Lab Share` - results_2015$`Lab Share`

# Compute responses for 2019
response_2019 <- data.frame(matrix(nrow = 543, ncol = 7))
names(response_2019) <- c('Constituency', 'Region', 'y_1', 'y_2', 'y_3', 'y_4', 'y_5')
...
response_2019$y_2 <- results_2019$`RFM Share` - results_2017$`RFM Share`
response_2019$y_3 <- results_2019$`LD Share` - results_2017$`LD Share`
response_2019$y_4 <- results_2019$`Con Share` - results_2017$`Con Share`
response_2019$y_5 <- results_2019$`Lab Share` - results_2017$`Lab Share`

# Compute responses for 2024
response_2024 <- data.frame(matrix(nrow = 543, ncol = 7))
names(response_2024) <- c('Constituency', 'Region', 'y_1', 'y_2', 'y_3', 'y_4', 'y_5')
...
response_2024$y_2 <- results_2024$`RFM Share` - results_2019$`RFM Share`
response_2024$y_3 <- results_2024$`LD Share` - results_2019$`LD Share`
response_2024$y_4 <- results_2024$`Con Share` - results_2019$`Con Share`
response_2024$y_5 <- results_2024$`Lab Share` - results_2024$`Lab Share`

# Combine responses into a single data frame and order by constituencies
...
# Combine results into a single data frame and order by constituencies
...
# Get results for before and after each election
...
# Add home ownership proportions to election data
...
# Add change in national vote share to change data frame
...
# Create data frame containing responses, covariates, change in covariates, results before and after, and factors for if a party ran before or not
election_data_training <- cbind(response[, 3:7], election_data_training[, 13:40], election_change_data[, 3:19], results_2[, c(1, 13:17)], results_3[, c(3, 13:17)])
...
election_data_training$Blue_Wall <- as.factor(as.numeric(election_data_training$Constituency %in% blue_wall$Constituency))
...
write.csv(election_data_training, 'Adapted CF Model Data.csv')
