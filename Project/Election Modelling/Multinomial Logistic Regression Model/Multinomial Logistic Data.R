...
# This file prepares the response data and other covariates for the multinomial logistic regression model
...
# Load general election results for new boundaries for 2010 to 2024
...
# Compute vote share with votes for other parties removed for each election
...
# Compute national vote share in England for major parties
...
# Compute the responses for each model in multinomial logistic regression
results_2010$`Lab Ratio` <- log((results_2010$`Lab Share`) / results_2010$`Con Share`)
results_2015$`Lab Ratio` <- log((results_2015$`Lab Share`) / results_2015$`Con Share`)
results_2017$`Lab Ratio` <- log((results_2017$`Lab Share`) / results_2017$`Con Share`)
results_2019$`Lab Ratio` <- log((results_2019$`Lab Share`) / results_2019$`Con Share`)
results_2024$`Lab Ratio` <- log((results_2024$`Lab Share`) / results_2024$`Con Share`)

results_2010$`LD Ratio` <- log((results_2010$`LD Share`) / results_2010$`Con Share`)
results_2015$`LD Ratio` <- log((results_2015$`LD Share`) / results_2015$`Con Share`)
results_2017$`LD Ratio` <- log((results_2017$`LD Share`) / results_2017$`Con Share`)
results_2019$`LD Ratio` <- log((results_2019$`LD Share`) / results_2019$`Con Share`)
results_2024$`LD Ratio` <- log((results_2024$`LD Share`) / results_2024$`Con Share`)

results_2010$`RFM Ratio` <- log((results_2010$`RFM Share`) / results_2010$`Con Share`)
results_2015$`RFM Ratio` <- log((results_2015$`RFM Share`) / results_2015$`Con Share`)
results_2017$`RFM Ratio` <- log((results_2017$`RFM Share`) / results_2017$`Con Share`)
results_2019$`RFM Ratio` <- log((results_2019$`RFM Share`) / results_2019$`Con Share`)
results_2024$`RFM Ratio` <- log((results_2024$`RFM Share`) / results_2024$`Con Share`)

results_2010$`GRN Ratio` <- log((results_2010$`GRN Share`) / results_2010$`Con Share`)
results_2015$`GRN Ratio` <- log((results_2015$`GRN Share`) / results_2015$`Con Share`)
results_2017$`GRN Ratio` <- log((results_2017$`GRN Share`) / results_2017$`Con Share`)
results_2019$`GRN Ratio` <- log((results_2019$`GRN Share`) / results_2019$`Con Share`)
results_2024$`GRN Ratio` <- log((results_2024$`GRN Share`) / results_2024$`Con Share`)

# Create data frame for 2015 responses
response_2015 <- data.frame(matrix(nrow = 543, ncol = 11))
names(response_2015) <- c('Constituency', 'Year', 'y_1', 'y_2', 'y_3', 'y_4', 'Con', 'Lab', 'LD', 'RFM', 'Other')

response_2015$Constituency <- results_2015$Constituency
response_2015$Year <- 2015
response_2015$y_1 <- results_2015$`GRN Ratio`
response_2015$y_2 <- results_2015$`RFM Ratio`
response_2015$y_3 <- results_2015$`LD Ratio`
response_2015$y_4 <- results_2015$`Lab Ratio`
response_2015$Con <- con_2015
response_2015$Lab <- lab_2015
response_2015$LD <- ld_2015
response_2015$RFM <- rfm_2015
response_2015$GRN <- grn_2015

# Create data frame for 2017 responses
response_2017 <- data.frame(matrix(nrow = 543, ncol = 11))
names(response_2017) <- c('Constituency', 'Year', 'y_1', 'y_2', 'y_3', 'y_4', 'Con', 'Lab', 'LD', 'RFM', 'Other')

response_2017$Constituency <- results_2017$Constituency
response_2017$Year <- 2017
response_2017$y_1 <- results_2017$`GRN Ratio`
response_2017$y_2 <- results_2017$`RFM Ratio`
response_2017$y_3 <- results_2017$`LD Ratio`
response_2017$y_4 <- results_2017$`Lab Ratio`
response_2017$Con <- con_2017
response_2017$Lab <- lab_2017
response_2017$LD <- ld_2017
response_2017$RFM <- rfm_2017
response_2017$GRN <- grn_2017

# Create data frame for 2019 responses
response_2019 <- data.frame(matrix(nrow = 543, ncol = 11))
names(response_2019) <- c('Constituency', 'Year', 'y_1', 'y_2', 'y_3', 'y_4', 'Con', 'Lab', 'LD', 'RFM', 'Other')

response_2019$Constituency <- results_2019$Constituency
response_2019$Year <- 2019
response_2019$y_1 <- results_2019$`GRN Ratio`
response_2019$y_2 <- results_2019$`RFM Ratio`
response_2019$y_3 <- results_2019$`LD Ratio`
response_2019$y_4 <- results_2019$`Lab Ratio`
response_2019$Con <- con_2019
response_2019$Lab <- lab_2019
response_2019$LD <- ld_2019
response_2019$RFM <- rfm_2019
response_2019$GRN <- grn_2019

# Create data frame for 2024 responses
response_2024 <- data.frame(matrix(nrow = 543, ncol = 11))
names(response_2024) <- c('Constituency', 'Year', 'y_1', 'y_2', 'y_3', 'y_4', 'Con', 'Lab', 'LD', 'RFM', 'Other')

response_2024$Constituency <- results_2024$Constituency
response_2024$Year <- 2024
response_2024$y_1 <- results_2024$`GRN Ratio`
response_2024$y_2 <- results_2024$`RFM Ratio`
response_2024$y_3 <- results_2024$`LD Ratio`
response_2024$y_4 <- results_2024$`Lab Ratio`
response_2024$Con <- con_2024
response_2024$Lab <- lab_2024
response_2024$LD <- ld_2024
response_2024$RFM <- rfm_2024
response_2024$GRN <- grn_2024

# Combine results into single data frame and order by constituency and add year column
...
# Compute election results for before and after each election and change names of data frame
...
names(results_3) <- c('Constituency', 'Region', 'Winner', 'Majority', 'Con', 'Lab', 'LD', 'RFM', 'Green', 'Other', 'Total',
                      'Electorate', 'Con Share After', 'Lab Share After', 'LD Share After', 'RFM Share After',
                      'GRN Share After', 'Lab Ratio', 'LD Ratio', 'RFM Ratio', 'GRN Ratio' ,'Year')

# Combine responses into a single data frame and order by constituency
...
# Load all of the election demographic data
...
# Add national vote shares to election data
...
# Compute home ownership proportions
...
# Create data frame to store the change in covariates
election_change_data <- data.frame(matrix(nrow = 543 * 4, ncol = 14))
names(election_change_data) <- c('Constituency', 'Year', 'X18.29 Change', 'X30.49 Change', 'X50.64 Change', 'X65. Change', 'Level 4 Prop Change', 'No Qualifications Prop Change','White Change', 'Owner Prop Change', 'Private Prop Change', 'Social Prop Change', 'House Prices Change', 'Median Income Change')

# Compute the change in covariates
for (i in 1:543) {
  ...
  for (j in 1:4) {
    ...
    election_change_data$`House Prices Change`[k] <- election_data$home_prices[5 * i - 4 + j] - election_data$home_prices[5 * i - 5 + j]
    election_change_data$`Median Income Change`[k] <- election_data$`Adjusted Median Income`[5 * i - 4 + j] - election_data$`Adjusted Median Income`[5 * i - 5 + j]
...
# Compute change in national vote share
...
# Create an overall data frame containing responses, covariates, change in covariates, election results before and after, and add factor for if party stood before or not
...
# Remove Speaker's seat from data
...
# Save to a CSV file
write.csv(election_data_training, 'Multinomial Logistic Data.csv')
