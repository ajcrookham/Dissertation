...
# This file builds the CF models and computes the R squared values for the test set
...
# Load CF model data
election_data_training <- read.csv('CF Model Data.csv')
...
# Model for change in Green Party
...
full_1 <- lm(y_1 ~ (. - Year) + X18.29:`X18.29 Change` + X30.49:`X30.49 Change` + X50.64:`X50.64 Change` + X65.:`X65. Change`
             ...
             , data = election_data_training[!election_data_training$`GRN Share After` == 0, c(1, 5:16, 21:24, 30:44, 63)])
...
full_2 <- lm(y_2 ~ (. - Year) + X18.29:`X18.29 Change` + X30.49:`X30.49 Change` + X50.64:`X50.64 Change` + X65.:`X65. Change`
             ...
             + White:`White Change` + `Owner Prop`:`Owner Prop Change` + `Private Prop`:`Private Prop Change`
             + `Social Prop`:`Social Prop Change` + home_prices:`House Prices Change` + `Median Income`:`Median Income Change`
             , data = election_data_training[!election_data_training$`RFM Share After` == 0, c(2, 5:16, 21:24, 30:44, 63)])
...
full_3 <- lm(y_3 ~ (. - Year) + X18.29:`X18.29 Change` + X30.49:`X30.49 Change` + X50.64:`X50.64 Change` + X65.:`X65. Change`
             ...
             + White:`White Change` + `Owner Prop`:`Owner Prop Change` + `Private Prop`:`Private Prop Change`
             + `Social Prop`:`Social Prop Change` + home_prices:`House Prices Change` + `Median Income`:`Median Income Change`
             , data = election_data_training[!election_data_training$`LD Share After` == 0, c(3, 5:16, 21:24, 30:44, 63)])
...
# Model for Conservatives change
null_4 <- lm(y_4 ~ 1, data = election_data_training[!election_data_training$`Con Share After` == 0 & !election_data_training$`Lab Share After` == 0, ])
full_4 <- lm(y_4 ~ (. - Year) + X18.29:`X18.29 Change` + X30.49:`X30.49 Change` + X50.64:`X50.64 Change` + X65.:`X65. Change`
             + `Level 4 Prop`:`Level 4 Prop Change` + `No Qualifications Prop`:`No Qualifications Prop Change`
             + White:`White Change` + `Owner Prop`:`Owner Prop Change` + `Private Prop`:`Private Prop Change`
             + `Social Prop`:`Social Prop Change` + home_prices:`House Prices Change` + `Median Income`:`Median Income Change`
             , data = election_data_training[!election_data_training$`Con Share After` == 0 & !election_data_training$`Lab Share After` == 0, c(4, 5:16, 21:24, 30:44, 63)])

model_build <- step(null_4, scope = list(lower = null_4, upper = full_4), direction = 'both', trace = 0)
mod_4 <- lm(model_build$terms, data = election_data_training[!election_data_training$`Con Share After` == 0 & !election_data_training$`Lab Share After` == 0, ])
...
rfm_change <- (100 - grn) / 100 * predict(mod_2, newdata = election_data_testing)
rfm <- election_data_testing$`RFM Share` + rfm_change

ld_change <- (100 - grn - rfm) / 100 * predict(mod_3, newdata = election_data_testing)
ld <- election_data_testing$`LD Share` + ld_change

con_change <- (100 - grn - rfm - ld) / 100 * predict(mod_4, newdata = election_data_testing)
con <- election_data_testing$`Con Share` + con_change

lab <- 100 - grn - rfm - ld - con
lab_change <- lab - election_data_testing$`Lab Share`

# Set vote shares to zero for seats where the party did not run
for (i in 1:440) {
...
# Create data frame for predicted vote shares and actual vote shares of test set
...
cf_model_plot <- grid.arrange(con_plot, lab_plot, ld_plot, rfm_plot, grn_plot, legend, ncol = 2, nrow = 3,
                               layout_matrix = rbind(c(1,2), c(3,4), c(5,6)),
                               widths = c(2, 2), heights = c(4, 4, 4))


ggsave('CF Model Vote Share.png', cf_model_plot, width = 15.66, height = 12, units = 'cm')
...
cf_model_rsq <- c(con_rsq, lab_rsq, ld_rsq, rfm_rsq, grn_rsq)
cf_model_rsq

# Compute Con and Lab R squared values for 2015, 2017, and 2019
...
covariate.labels <- c('Intercept', '18 to 29', '30 to 49', '50 to 64', '65+', 'Change in 18 to 29',
                      ...
                      'Child Poverty', 'Fuel Poverty', 'Ethnicity',
                      'Home Owners', 'Private Housing', 'Social Housing',
                      'Home Owners Change', 'Private Housing Change', 'No RFM Before',
                      '18 to 29 Int.', 'Degree Int.', 'No Qual. Int.', 'No Qual. Int. 2',
                      'Median Income Int.', 'Median Income Int. 2', 'Home Owners Int.', 'Private Housing Int.')


covariate.stargazer <- c('Intercept', 'Change in Degree', 'Inflation', 'Unemployment',
                         'No Qualifications', 'Degree', '30 to 49', 'Change in Median Income', 'Fuel Poverty',
                         'No RFM Before', 'Social Housing', 'Home Owners Change', 'Median House Price Change',
                         'Private Housing Change', 'No Qual. Int.', '65+', 'Degree Int.', 'Home Owners Int.',
                         'Median Income Int.', 'Change in No Qual.', 'Ethnicity', 'Median House Price', 
                         '18 to 29', 'Child Poverty', 'Median Age', 'Median Income Int. 2', 'Private Housing',
                         'Change in 65+', 'Change in 50 to 64', 'Home Owners', 'Change in 18 to 29', '50 to 64',
                         'Median Income', 'No Qual. Int. 2', 'Private Housing Int.', '18 to 29 Int.')
